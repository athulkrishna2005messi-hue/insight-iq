...
type KpiFields = Pick<DashboardKpis, "activeUsers" | "new7d" | "churn30d" | "mrr" | "arpu" | "minutes">;
...
const KPI_TILES: KpiTile[] = [
  ...
  },
  {
    ...
    format: (value) => currencyWholeFormatter.format(value)
  },
  {
    key: "arpu",
    label: "ARPU",
    format: (value) => currencyPreciseFormatter.format(value)
  },
  {
    key: "minutes",
...
export default async function DashboardPage({ params }: { params: { companyId: string } }) {
  ...
  const trendDeltaPercent = firstTrend === 0 ? 0 : (trendDelta / firstTrend) * 100;
  ...
  return (
    ...
    >
      <header className="space-y-3" style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
        <span
          className="text-sm font-medium uppercase tracking-wide text-slate-500"
          style={{ fontSize: "12px", letterSpacing: "0.32em", textTransform: "uppercase", color: "#64748b" }}
        >
          Company overview
        </span>
        <h1
          className="text-3xl font-semibold tracking-tight text-slate-900"
          style={{ fontSize: "32px", fontWeight: 600, color: "#0f172a", letterSpacing: "-0.03em" }}
        >
        ...
        >
          Live KPIs aggregated from the mock data source. Use these numbers to explore the dashboard experience and validate integrations.
        </p>
      </header>
      ...
      <section>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" style={GRID_STYLE}>
          {KPI_TILES.map((tile) => {
            ...
            const formatted = tile.format ? tile.format(value) : numberFormatter.format(value);
            return (
              <div key={tile.key} className="rounded-xl border bg-white shadow-sm" style={CARD_STYLE}>
                <span
                  className="text-sm font-medium uppercase tracking-wide text-slate-500"
                  style={{ fontSize: "12px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.22em", color: "#64748b" }}
                >
                  {tile.label}
                </span>
                <span
      ...
      <section className="space-y-4" style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        <div className="flex items-start justify-between gap-4" style={{ display: "flex", justifyContent: "space-between", gap: "16px" }}>
          <div className="space-y-1" style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            <h2
            ...
            >
              Weekly active accounts over the past seven intervals with a simple sparkline approximation.
            </p>
          </div>
          <div className="text-right" style={{ textAlign: "right" }}>
            ...
            <div
              className="text-sm"
              style={{ fontSize: "13px", color: trendDelta >= 0 ? "#047857" : "#b91c1c" }}
            >
              {trendDelta >= 0 ? "▲" : "▼"} {percentFormatter.format(Math.abs(trendDeltaPercent))}% vs start
            </div>
          </div>
        </div>
        ...
        >
          <div className="flex flex-1 items-end gap-3" style={{ display: "flex", flex: 1, alignItems: "flex-end", gap: "12px", height: "100%" }}>
            {data.trend.map((value, index) => {
              const heightPercent = maxTrend === 0 ? 0 : (value / maxTrend) * 100;
              return (
                <div
                  key={`${value}-${index}`}
                  ...
                  style={{
                    ...
                    minWidth: "12px",
                    borderRadius: "12px",
                    background: "linear-gradient(180deg, rgba(59, 130, 246, 0.9), rgba(37, 99, 235, 0.85))",
          ...
          <div className="flex flex-col justify-between text-xs text-slate-500" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", fontSize: "12px", color: "#475569" }}>
